        ...
        tooltip: {
          callbacks: {
            label: function (context) {
              return (
                "Probability: " + (context.parsed.y * 100).toFixed(2) + "%"
              );
            },
          },
        },
...
function addChartPoint(probability) {
  ...
  if (probabilityHistory.length > 30) {
    probabilityHistory.shift();
  }

  if (probabilityChart) {
    probabilityChart.data.labels = probabilityHistory.map((p) => p.time);
    probabilityChart.data.datasets[0].data = probabilityHistory.map(
      (p) => p.probability
    );
...
function renderSimulation(data) {
  ...
  // Display final prior (from backend or calculate it)
  if (data.final_prior !== undefined) {
    finalPriorDisplay.textContent = (data.final_prior * 100).toFixed(2) + "%";
  } else {
    updateFinalPrior(data.global_prior, data.time_prior);
  }
  ...
  // Initialize weight controls from entity_type_weights or entity weights
  if (data.entity_type_weights) {
    Object.keys(weightControls).forEach((entityType) => {
      const controls = weightControls[entityType];
      if (controls && data.entity_type_weights[entityType] !== undefined) {
        const weight = data.entity_type_weights[entityType];
        if (controls.slider) controls.slider.value = weight;
        if (controls.input) controls.input.value = weight;
        if (controls.display) controls.display.textContent = weight.toFixed(2);
      }
    });
  } else if (data.entities) {
    // Fallback: extract weights from entities
    const entityTypeWeights = {};
    data.entities.forEach((entity) => {
      const entityType = entity.type;
      if (entityType && !entityTypeWeights[entityType]) {
        entityTypeWeights[entityType] = entity.weight;
      }
    });
    Object.keys(weightControls).forEach((entityType) => {
      const controls = weightControls[entityType];
      if (controls && entityTypeWeights[entityType] !== undefined) {
        const weight = entityTypeWeights[entityType];
        if (controls.slider) controls.slider.value = weight;
        if (controls.input) controls.input.value = weight;
        if (controls.display) controls.display.textContent = weight.toFixed(2);
      }
...
function updateCombinedPrior(globalPrior, timePrior) {
  ...
  // Also update final prior
  updateFinalPrior(globalPrior, timePrior);
}

// Update final prior display (after PRIOR_FACTOR and clamping)
function updateFinalPrior(globalPrior, timePrior) {
  // Constants matching backend
  const PRIOR_FACTOR = 1.05;
  const MIN_PRIOR = 0.1;
  const MAX_PRIOR = 0.99;

  // Calculate combined prior (same logic as updateCombinedPrior)
  const timeWeight = 0.2;
  const areaWeight = 1.0 - timeWeight;
  ...
  // Apply PRIOR_FACTOR
  let finalPrior = combinedPrior * PRIOR_FACTOR;

  // Clamp to MIN_PRIOR and MAX_PRIOR
  finalPrior = Math.max(MIN_PRIOR, Math.min(MAX_PRIOR, finalPrior));

  finalPriorDisplay.textContent = (finalPrior * 100).toFixed(2) + "%";
}
  ...
  entities.forEach((entity) => {
    ...
    if (isNumeric) {
      ...
      input.addEventListener("change", () =>
        handleNumericUpdate(entity.entity_id, input.value)
      );
    } else {
      const checkbox = sensorItem.querySelector('input[type="checkbox"]');
      checkbox.addEventListener("change", () =>
        handleToggle(entity.entity_id, checkbox.checked)
      );
    }
  });
}
  ...
  try {
    ...
    if (sensorItem) {
      ...
      if (stateSpan) {
        ...
        stateSpan.className = `sensor-state ${
          isActive ? "active" : "inactive"
        }`;
      }
...
async function handlePriorUpdate() {
  ...
  priorUpdateTimeout = setTimeout(async () => {
    try {
      ...
      // Update final prior display
      if (data.final_prior !== undefined) {
        finalPriorDisplay.textContent =
          (data.final_prior * 100).toFixed(2) + "%";
      } else {
        updateFinalPrior(
          parseFloat(globalPriorSlider.value),
          parseFloat(timePriorSlider.value)
        );
      }
    } catch (error) {
...
function setupWeightHandlers() {
  // Set up handlers for each entity type
  Object.keys(weightControls).forEach((entityType) => {
    const controls = weightControls[entityType];
    if (!controls.slider || !controls.input || !controls.display) {
    ...
    controls.slider.addEventListener("input", (e) => {
      ...
      controls.display.textContent = value.toFixed(2);
      handleWeightUpdate();
    });
...
async function handleWeightUpdate() {
  ...
  weightUpdateTimeout = setTimeout(async () => {
    try {
      // Collect all current weight values
      const weights = {};
      Object.keys(weightControls).forEach((entityType) => {
        const controls = weightControls[entityType];
        if (controls.input) {
          const value = parseFloat(controls.input.value);
          if (!isNaN(value)) {
            weights[entityType] = value;
          }
        }
      });

      const response = await fetch("/api/update-weights", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        showError(data.error || "Failed to update weights");
        return;
      }
      ...
      // Update weight displays from response (in case of clamping)
      if (data.weights) {
        Object.keys(data.weights).forEach((entityType) => {
          const controls = weightControls[entityType];
          if (controls) {
            const weight = data.weights[entityType];
            if (controls.slider) controls.slider.value = weight;
            if (controls.input) controls.input.value = weight;
            if (controls.display)
              controls.display.textContent = weight.toFixed(2);
          }
        });
      }
    } catch (error) {
      showError("Error updating weights: " + error.message);
    }
  }, 300); // 300ms debounce
}

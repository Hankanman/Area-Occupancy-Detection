...
| Bedroom (SLEEPING purpose) outside the configured sleep window: is a **custom** half-life being silently replaced by the purpose's `awake_half_life` (620s)? | See burned lesson #2 below (issue #481) — compare the diagnostics `config.decay` stored half-life against `Purpose.awake_half_life`; if they differ and the room still takes ~620s (10 min) to clear, this bug is present |
...
| (feat/adjacent-areas, **not yet merged as of 2026-07-06** — verify with `gh pr view 454`) Is an adjacent area's silence extending this area's decay? | Diagnostics `current.adjacency.decay_modifier` — capped at 1.75× base half-life |
...
| Is `0.99` from **global-prior denominator inflation** during a long quiet stretch? | See burned lesson #3 below (issue #483) — check whether the area was quiet (no occupied intervals) for more than ~1 hour before recent analysis cycles; if `global_prior` keeps climbing every hourly recalculation during quiet periods, this bug is present |
...
2. **Issue #481 (2026-05-20, open — fix in PR #493, unmerged as of 2026-07-06)**: the identical bug class recurred in a *different* code path. `Decay.half_life` implements a sleep/awake split for SLEEPING-purpose areas: use the configured half-life during the sleep window, switch to the purpose's `awake_half_life` (620s) outside it. The switch applied **unconditionally** — so a user's custom 10-second half-life got silently replaced by 620s (a ~15 minute clear-out) during every waking hour, with no way to keep a short custom half-life active outside the sleep window. Reported symptom: bedroom that should clear in ~10s instead took ~15 minutes.
3. **PR #493 (open, verify with `gh pr view 493`)**: fix mirrors #440's pattern exactly — the awake/sleep switch is restricted to only engage when the area's half-life still **equals the purpose's own default** (i.e., the user left it on auto). A custom value now wins at all times of day.
...
**Discriminating experiment**: for a SLEEPING-purpose area, set a custom half-life that is *not* 1200s (the purpose default) or 620s (`awake_half_life`), then check the diagnostics `config.decay` value both inside and outside the configured sleep window — it should read the same custom value in both cases. Also check any *other* purpose combined with a numerically coincidental custom value (e.g. Living Room + 600s, Office's default) to catch a #439-style regression.
...
Verified: `custom_components/area_occupancy/data/purpose.py` (`is_purpose_half_life` docstring explicitly cites #439 and describes the legacy bug); `custom_components/area_occupancy/data/decay.py` lines ~58-90 (`half_life` property, unconditional `return self._purpose.awake_half_life` outside the sleep window — confirmed present, unfixed, on `main` as of 2026-07-06); `config_flow.py` ~line 1539 (`Purpose.is_purpose_half_life(user_set_decay, purpose)`, comment citing #439); `gh pr view 440`, `gh pr view 493` (state OPEN, body: "mirrors the custom-vs-default semantics established for #440").

## Burned lesson #3: prior pinned at 0.99 (#483 quiet-tail denominator)

**Root cause**: `PriorAnalyzer.calculate_and_update_prior()` computes `global_prior = occupied_duration / actual_period_duration`. The code sets `actual_period_end = last_interval_end` (instead of `now`) whenever the area has been quiet for **more than 3600 seconds (1 hour)** before the analysis run. This truncates the denominator's "known unoccupied" tail while the numerator keeps counting all historically occupied seconds — every hourly analysis cycle that runs during an overnight or weekend quiet stretch pushes `global_prior` a little higher, and repeated cycles compound the effect until it saturates at the `MAX_PRIOR = 0.99` clamp.
...
**Wrong path people take**: treating a 0.99-pinned prior as a sensor/weight/threshold misconfiguration and tuning those instead. The actual defect is in the *learning* code's period-window math, not in anything sensor-facing — nothing in the entity config is wrong.

**Discriminating experiment**: check whether the area has had any extended quiet stretch (no occupied intervals for >1h) before recent analysis cycles, and whether `global_prior` has been monotonically climbing cycle over cycle rather than settling. Diagnostics' `prior.global_prior` field plus manual arithmetic (`occupied_duration_seconds / total_period_seconds` over the *full*, non-truncated window) will disagree with the stored value if this bug is active.

**Status as of 2026-07-06**: the bug is confirmed present on `main` (`data/analysis.py`, the `if (now - last_interval_end).total_seconds() > 3600: actual_period_end = last_interval_end` branch). **PR #491 is open, not merged** — verify current state with `gh pr view 491`. Its fix sets `actual_period_end` to always be `now`, removing the truncation entirely (the original branch's stated purpose — guarding a degenerate just-after-startup period — is already covered by a separate `actual_period_duration <= 0` guard).
...
Verified: `custom_components/area_occupancy/data/analysis.py` lines ~495-498 (`if (now - last_interval_end).total_seconds() > 3600: actual_period_end = last_interval_end`); `gh issue view 483` (open); `gh pr view 491` (state OPEN, body confirms root cause and fix); `gh pr view 246`, `gh pr view 251`, `gh pr view 266`, `gh pr view 356` (all MERGED, all prior-calculation titles).

---

## Provenance and maintenance

Date-stamped 2026-07-06, integration version 2026.5.17 (`git log -1 --oneline` on `main` → `704c89e chore: bump version to 2026.5.17 (#475)`). All line numbers and constant values in this file were read directly from `main` via `git show main:<path>` (this session's working tree was checked out to `feat/adjacent-areas`, an unmerged branch — every fact above was cross-checked against `main`, not the working tree, unless explicitly marked as pending/unmerged).
...
- **Open/merged status of every cited PR/issue** (#440, #481, #483, #486, #488, #491, #493, #454, #386/#379, #439, #444, #445, #446, #463, #465, #466, #467, #468, #472, #473, #474): `gh pr view <n> --json state,mergedAt,baseRefName` / `gh issue view <n> --json state` — several (notably #491, #493, #454) are explicitly still open as of this date and must be re-checked before being described as shipped.
- **Integration version / commit**: `git log -1 --oneline` (on `main`, not necessarily the current working branch — check `git branch --show-current` first).

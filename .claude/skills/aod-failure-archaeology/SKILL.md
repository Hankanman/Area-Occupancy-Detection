...
**Fix — PR #493 "respect custom half-life for Bedroom purpose" (state: OPEN, not yet merged to `main` as of 2026-07-06 — verify with `gh pr view 493`):** adds the guard `if self._base_half_life != self._purpose.half_life: return self._base_half_life` — any half-life differing from the purpose default is treated as a deliberate user override and the awake/asleep switch is skipped entirely; the switch only engages when the half-life equals the purpose default (i.e., the user left it on auto), explicitly modeled on #440's fix pattern. Verified in the PR diff (`git diff origin/main origin/fix/bedroom-half-life-override -- custom_components/area_occupancy/data/decay.py`): the guard is a single added `if` before the sleep-window check.

**Status: OPEN** (PR #493 CI-green, awaiting maintainer merge as of 2026-07-06).
...
Evidence: issue #439 (closed 2026-04-17); PR #440 (merged 2026-04-17); issue #481 (open, 2026-05-20); PR #493 (open, body: "mirrors the custom-vs-default semantics established for #440"); `custom_components/area_occupancy/data/decay.py:59-91` on `main` (current unconditional-switch bug, read directly) vs. `origin/fix/bedroom-half-life-override` diff (fix, read directly).
...
**Fix — PR #491 "keep quiet tail in global prior denominator" (state: OPEN, not yet merged to `main` as of 2026-07-06 — verify with `gh pr view 491`):** `actual_period_end` is now unconditionally `now`; the pre-existing `actual_period_duration <= 0` guard already covers the degenerate-denominator case the old conditional was trying to protect. The corrected test now asserts `0.25`, and a new regression test `test_quiet_tail_included_in_denominator` was added for the overnight-quiet scenario specifically. Verified via `git diff origin/main origin/fix/global-prior-quiet-tail`.

**Status: OPEN** (PR #491 CI-green, awaiting maintainer merge as of 2026-07-06). Note this bug directly matches the maintainer's stated "costliest past failure": prior pinned at 0.99.
...
Evidence: issue #483 (open, 2026-05-29, root-cause credit to @mscharwere); PR #491 (open, body cites #483, "Credit to @mscharwere for the precise root-cause analysis"); `git diff origin/main origin/fix/global-prior-quiet-tail -- custom_components/area_occupancy/data/analysis.py` and `-- tests/test_data_analysis.py` (both read directly, confirms the exact removed conditional and the corrected test assertion 0.99→0.25).
...
**Fix — PR #492 "detect sleep when person presence is unknown" (state: OPEN, not yet merged to `main` as of 2026-07-06 — verify with `gh pr view 492`):** adds a `_person_home_state()` helper returning `True`/`False`/`None` (tri-state: `None` = indeterminate — `unknown`/`unavailable`/missing entity). `_evaluate_sleep_state()` now skips a person only when **definitively away**; when presence is unknown, sleep sensors are trusted directly. 13 new tests added (the sensor had zero prior test coverage).

**Status: OPEN** (PR #492 CI-green, awaiting maintainer merge as of 2026-07-06).
...
Evidence: PR #375 (merged 2026-02-22, CONF_VERSION 16→17); issue #464 (open, 2026-05-07, reporter's own attribute dump showing `active: true` / `sleeping: false`); PR #492 (open, body: "Fixes #464 ... exactly as @laszlojakab traced in the issue"); `git show 368c03a9:custom_components/area_occupancy/binary_sensor.py` (fallback-already-present check, read directly at the commit cited by a since-superseded diagnosis).
...
Evidence: issue #487 (closed 2026-07-06); PR #489 (merged 2026-07-06, commit `704c89e`; maintainer review body quotes the exact blog post title/date/URL); `pyproject.toml:26` (`"homeassistant==2026.2.2"`, read directly).

---

## SAGA 8 — Adjacent-areas: a feature dormant since 2025, then built in five phases, still unmerged
...
**Implementation — PR #454 "feat: adjacent-areas (next-door room influence)" (created 2026-05-03; state as of 2026-07-06: OPEN, not merged to main — re-verify with `gh pr view 454`), a 5-phase build:**
...
4. Bayesian wiring (**folded in from a separate stacked PR, #456**, merged 2026-07-06 — but merged into the `feat/adjacent-areas` branch, not `main`) — a logit-additive boost in `Area.probability()` plus a decay-half-life stretch modifier (capped at 1.75×) for areas whose adjacent exits have been quiet.
5. Tests + docs top-up.
...
**CodeRabbit review nitpicks confirmed still relevant (verified via `gh api .../pulls/454/reviews`):** (1) `db/relationships.py`'s module docstring still states "no adjacent-areas UI/producer exists," which became false once this PR landed — a stale doc-comment trap if anyone reads that file in isolation; (2) no regression test existed for `CONF_ADJACENT_AREAS` being a malformed non-list value at the time of review (later addressed by `_normalize_adjacent_areas` defensive coercion, also reviewed).

**Tunables are explicitly unvalidated (verified, `const.py:189-221`, comment: "First-pass values; tune from real data once Phase 3 is collecting transitions"):** `ADJACENCY_TRANSITION_WINDOW_S=60`, `ADJACENCY_RECENCY_HALF_LIFE_DAYS=30`, `ADJACENCY_TRAJECTORY_WINDOW_S=300`, `ADJACENCY_BOOST_GAIN=0.5`, `ADJACENCY_DECAY_MODIFIER_GAIN=0.75`, `ADJACENCY_DECAY_MODIFIER_MAX=1.75`, and four minimum-observation smoothing thresholds (5/20/50/20). No commit or test exercises these against real recorder data — only synthetic/mocked entities in tests.

**Status: OPEN.** PR #454 is CI-green and mergeable as of the last check, but is a large (4,100+/−100 line), multi-phase, still-unmerged feature. Do not treat any adjacent-areas behavior as shipped/available to users until #454 merges to `main` — check `gh pr view 454 --json state,mergedAt` before relying on this.

**The lesson:** this is the single largest unmerged feature in the repo's history and a case study in "features can sit dormant in the schema for months before anyone builds the rest" — if you're investigating what looks like dead/unused schema (columns or tables with no readers), check open PRs and discussions before assuming it's abandoned; it may be a future feature's foundation laid early. Also: when a feature request suggests a specific mechanism (#431's hand-tuned confidence sliders), the shipped implementation is free to choose a different mechanism (learned influence) that satisfies the underlying need — document that divergence explicitly in the PR, as #454 did, so the original requester's mental model doesn't silently mismatch what shipped.

Evidence: `git show a99ad49:custom_components/area_occupancy/db/schema.py` (schema fields present 2025-11-17, read directly); discussion #431 (open, 0 comments, verified via `gh api repos/.../discussions/431`); PR #454 (open, full 5-phase body read directly); PR #456 (state MERGED, `mergedAt: 2026-07-06T10:01:29Z`, base branch `feat/adjacent-areas` — merged into the feature branch, not main); `custom_components/area_occupancy/const.py:189-221` (tunables, read directly); PR #454 review comments (`gh api repos/Hankanman/Area-Occupancy-Detection/pulls/454/reviews`, CodeRabbit nitpicks on `db/relationships.py` docstring and `_normalize_adjacent_areas` docstring accuracy).
...
Compiled 2026-07-06 against integration version 2026.5.17 (`main` branch — note the working tree this skill was authored from was actually checked out to `feat/adjacent-areas`, 28 commits ahead of `main`; every `main`-specific claim above was verified with `git show origin/main:<path>` or `gh pr view --json baseRefName,mergedAt`, not by trusting the local working tree).
...
- **Adjacent-areas merge status and tunables:** `gh pr view 454 --json state,mergedAt`; `git show origin/main:custom_components/area_occupancy/const.py | grep -n ADJACENCY_`

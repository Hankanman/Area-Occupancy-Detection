...
- **Single maintainer, no branch protection.** Verified directly: `gh api repos/Hankanman/Area-Occupancy-Detection/branches/main/protection` returns `404 Branch not protected`. Nothing prevents a direct push to `main` bypassing review or CI. Combined with the project being a single-maintainer effort (per this library's own premise), this is a real bus-factor and accidental-force-push risk — be conservative with anything that touches `main` directly.

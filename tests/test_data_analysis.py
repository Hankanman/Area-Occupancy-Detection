...
class TestPriorAnalyzerEdgeCases:
    ...
    def test_get_interval_aggregates_python_empty_intervals(
        self, test_db: AreaOccupancyDB
    ) -> None:
        """Test _get_interval_aggregates_python with empty intervals."""
        coordinator = test_db.coordinator
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)

        # Mock empty intervals
...
class TestPriorAnalyzerHelperMethods:
    """Test PriorAnalyzer helper methods."""

    def test_build_base_filters_with_area_name(
        self, coordinator: AreaOccupancyCoordinator
    ) -> None:
        """Test _build_base_filters with area_name."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = coordinator.db
    ...
    def test_build_motion_query(
        ...
        """Test _build_motion_query constructs query correctly."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = coordinator.db
        session = db_test_session
        lookback_date = dt_util.utcnow() - timedelta(days=90)
        base_filters = analyzer._build_base_filters(db, lookback_date)

        query = analyzer._build_motion_query(session, db, base_filters)

        # Verify query structure by checking it has expected attributes
        assert query is not None
        # Query should filter by motion type and "on" state
        assert hasattr(query, "filter") or hasattr(query, "where")

    def test_build_media_query(
        self, coordinator: AreaOccupancyCoordinator, db_test_session
    ) -> None:
        """Test _build_media_query with sensor_ids filtering."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = coordinator.db
        session = db_test_session
        lookback_date = dt_util.utcnow() - timedelta(days=90)
        base_filters = analyzer._build_base_filters(db, lookback_date)
        sensor_ids = ["media_player.test1", "media_player.test2"]

        query = analyzer._build_media_query(session, db, base_filters, sensor_ids)

        assert query is not None
        # Query should filter by media type and sensor_ids
        assert hasattr(query, "filter") or hasattr(query, "where")

    def test_build_appliance_query(
        self, coordinator: AreaOccupancyCoordinator, db_test_session
    ) -> None:
        """Test _build_appliance_query with sensor_ids filtering."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = coordinator.db
        session = db_test_session
        lookback_date = dt_util.utcnow() - timedelta(days=90)
        base_filters = analyzer._build_base_filters(db, lookback_date)
        sensor_ids = ["switch.test1", "switch.test2"]

        query = analyzer._build_appliance_query(session, db, base_filters, sensor_ids)

        assert query is not None
        # Query should filter by appliance type and sensor_ids
        assert hasattr(query, "filter") or hasattr(query, "where")

    def test_execute_union_queries_single_query(
        self, coordinator: AreaOccupancyCoordinator, db_test_session
    ) -> None:
        """Test _execute_union_queries with single query."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = coordinator.db
        session = db_test_session
        lookback_date = dt_util.utcnow() - timedelta(days=90)
        base_filters = analyzer._build_base_filters(db, lookback_date)
        motion_query = analyzer._build_motion_query(session, db, base_filters)

        # Mock query result
        mock_result = (dt_util.utcnow(), dt_util.utcnow() + timedelta(hours=1))
        with (
            patch.object(motion_query, "order_by", return_value=motion_query),
            patch.object(motion_query, "all", return_value=[mock_result]),
        ):
            results = analyzer._execute_union_queries(session, db, [motion_query])

        # Results should be converted to (start, end, sensor_type) format
        assert isinstance(results, list)

    def test_execute_union_queries_multiple_queries(
        self, coordinator: AreaOccupancyCoordinator, db_test_session
    ) -> None:
        """Test _execute_union_queries with multiple queries using UNION ALL."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = coordinator.db
        session = db_test_session
        ...
        # Create second query (media)
        media_sensor_ids = ["media_player.test"]
        media_query = analyzer._build_media_query(
            session, db, base_filters, media_sensor_ids
        )
    ...
    def test_process_query_results(self, coordinator: AreaOccupancyCoordinator) -> None:
        """Test _process_query_results separates by sensor type."""
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)

        now = dt_util.utcnow()
        results = [
            (now, now + timedelta(hours=1), "motion"),
            (now + timedelta(hours=1), now + timedelta(hours=2), "motion"),
            (now + timedelta(hours=2), now + timedelta(hours=3), "media"),
            (now + timedelta(hours=3), now + timedelta(hours=4), "appliance"),
        ]

        all_intervals, motion_raw, media_count, appliance_count = (
            analyzer._process_query_results(results)
        )

        assert len(all_intervals) == 4
        assert len(motion_raw) == 2
        assert media_count == 1
        assert appliance_count == 1

    def test_get_time_bounds_from_session_with_entity_ids(
        self, test_db: AreaOccupancyDB, db_test_session
    ) -> None:
        """Test _get_time_bounds_from_session with entity_ids filtering."""
        coordinator = test_db.coordinator
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = test_db
        ...
        entity_ids = ["binary_sensor.test_motion"]
        first, last = analyzer._get_time_bounds_from_session(session, db, entity_ids)

        assert first is not None
        assert last is not None

    def test_get_time_bounds_from_session_without_entity_ids(
        self, test_db: AreaOccupancyDB, db_test_session
    ) -> None:
        """Test _get_time_bounds_from_session without entity_ids (area-based)."""
        coordinator = test_db.coordinator
        area_name = coordinator.get_area_names()[0]
        analyzer = PriorAnalyzer(coordinator, area_name)
        db = test_db
        ...
        first, last = analyzer._get_time_bounds_from_session(session, db, None)

        assert first is not None
        assert last is not None

    def test_get_occupied_intervals_from_session_with_media_appliance(
        self, test_db: AreaOccupancyDB, db_test_session
    ) -> None:
        """Test _get_occupied_intervals_from_session with media and appliance."""
        coordinator = test_db.coordinator
        area_name = coordinator.get_area_names()[0]
        area = coordinator.get_area_or_default(area_name)
        area.config.sensors.media = ["media_player.test_tv"]
        ...
        # Create test entities
        motion_entity = db.Entities(
            entry_id=coordinator.entry_id,
            area_name=area_name,
            entity_id="binary_sensor.test_motion",
            entity_type=InputType.MOTION,
        )
        media_entity = db.Entities(
            entry_id=coordinator.entry_id,
            area_name=area_name,
            entity_id="media_player.test_tv",
            entity_type=InputType.MEDIA,
        )
        appliance_entity = db.Entities(
            entry_id=coordinator.entry_id,
            area_name=area_name,
            entity_id="switch.test_appliance",
            entity_type=InputType.APPLIANCE,
        )
        session.add_all([motion_entity, media_entity, appliance_entity])
        session.commit()

        intervals = analyzer._get_occupied_intervals_from_session(
            session,
            db,
            lookback_date,
            include_media=True,

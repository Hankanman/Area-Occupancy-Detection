...
class TestFilterIntervals:
    ...
    def test_filter_valid_intervals(self) -> None:
        ...
        assert (
            len(filtered) == 1
        )  # Only the "on" interval should remain (filter_intervals focuses on motion sensors)
...
class TestGetIntervalsHybrid:
    """Test get_intervals_hybrid function."""

    @pytest.mark.asyncio
    async def test_database_only(self, mock_coordinator: Mock) -> None:
        """Test getting intervals from database only."""
        start_time = dt_util.utcnow() - timedelta(hours=1)
        end_time = dt_util.utcnow()
        ...
        # Mock the coverage calculation to return 1.0 (full coverage) so it doesn't call recorder
        with patch(
            ...
            assert result == mock_intervals

    @pytest.mark.asyncio
    async def test_database_error_fallback(self, mock_coordinator: Mock) -> None:
        """Test fallback to recorder when database fails."""
        start_time = dt_util.utcnow() - timedelta(hours=1)
        end_time = dt_util.utcnow()
    ...
    @pytest.mark.asyncio
    async def test_partial_coverage_supplement(self, mock_coordinator: Mock) -> None:
        """Test supplementing partial database coverage with recorder data."""
        start_time = dt_util.utcnow() - timedelta(hours=1)
        end_time = dt_util.utcnow()

        # Database provides only 50% coverage
        mock_db_intervals = [
            StateInterval(
                start=start_time,
                end=start_time + timedelta(minutes=30),
        ...
        mock_coordinator.sqlite_store.get_historical_intervals = AsyncMock(
            return_value=mock_db_intervals
        )
        mock_coordinator.hass = Mock()
    ...
    async def test_no_sqlite_store(self, mock_coordinator: Mock) -> None:
        """Test when sqlite_store is None."""
        start_time = dt_util.utcnow() - timedelta(hours=1)
        end_time = dt_util.utcnow()

        mock_coordinator.sqlite_store = None
        mock_coordinator.hass = Mock()
...
class TestCalculateTimeCoverage:
    """Test _calculate_time_coverage function."""

    def test_empty_intervals(self) -> None:
        """Test coverage calculation with empty intervals."""
        start_time = dt_util.utcnow()
        end_time = start_time + timedelta(hours=1)

        # Import the function directly
        from custom_components.area_occupancy.utils import _calculate_time_coverage

        coverage = _calculate_time_coverage([], start_time, end_time)
        assert coverage == 0.0
    ...
    def test_full_coverage(self) -> None:
        ...
        coverage = _calculate_time_coverage(intervals, start_time, end_time)
        assert coverage == 1.0
    ...
    def test_overlapping_intervals(self) -> None:
        """Test coverage calculation with overlapping intervals."""
        start_time = dt_util.utcnow()
        end_time = start_time + timedelta(hours=1)
        ...
        intervals = [
            StateInterval(
                start=start_time,
                end=start_time + timedelta(minutes=40),
                state="on",
                entity_id="binary_sensor.test",
            ),
            StateInterval(
                start=start_time + timedelta(minutes=20),
                end=end_time,
                state="off",
                entity_id="binary_sensor.test",
            ),
        ...
        coverage = _calculate_time_coverage(intervals, start_time, end_time)
        assert coverage == 1.0  # Full coverage due to overlap

    def test_intervals_outside_window(self) -> None:
        """Test coverage calculation with intervals outside the window."""
        start_time = dt_util.utcnow()
        end_time = start_time + timedelta(hours=1)
        ...
        coverage = _calculate_time_coverage(intervals, start_time, end_time)
        assert coverage == 0.5  # Only half covered within window

    def test_zero_time_window(self) -> None:
        """Test coverage calculation with zero time window."""
        start_time = dt_util.utcnow()
...
class TestMergeIntervals:
    """Test _merge_intervals function."""

    def test_empty_intervals(self) -> None:
        """Test merging empty intervals."""
        from custom_components.area_occupancy.utils import _merge_intervals

        result = _merge_intervals([])
        assert result == []
    ...
    def test_adjacent_intervals_same_state(self) -> None:
        """Test merging adjacent intervals with same state."""
        start_time = dt_util.utcnow()
        intervals = [
            StateInterval(
                start=start_time,
        ...
        result = _merge_intervals(intervals)
        assert len(result) == 1
        assert result[0]["start"] == start_time
        assert result[0]["end"] == start_time + timedelta(minutes=60)
        assert result[0]["state"] == "on"

    def test_overlapping_intervals_same_state(self) -> None:
        """Test merging overlapping intervals with same state."""
        start_time = dt_util.utcnow()
        intervals = [
            StateInterval(
                start=start_time,
    ...
    def test_different_states_no_merge(self) -> None:
        """Test that intervals with different states are not merged."""
        start_time = dt_util.utcnow()
        intervals = [
            StateInterval(
                start=start_time,
    ...
    def test_different_entities_no_merge(self) -> None:
        """Test that intervals with different entities are not merged."""
        start_time = dt_util.utcnow()
        intervals = [
            StateInterval(
                start=start_time,
        ...
        assert result[1]["entity_id"] == "binary_sensor.test2"

    def test_complex_merge_scenario(self) -> None:
        """Test complex merging scenario with multiple intervals."""
        start_time = dt_util.utcnow()
        intervals = [
            StateInterval(
                start=start_time,
                end=start_time + timedelta(minutes=20),
                state="on",
                entity_id="binary_sensor.test",
            ),
            StateInterval(
            ...
            StateInterval(
                start=start_time + timedelta(minutes=40),
                end=start_time + timedelta(minutes=60),
                state="on",
                entity_id="binary_sensor.test",
            ),
            StateInterval(
                ...
                end=start_time + timedelta(minutes=80),
                state="on",
                entity_id="binary_sensor.test",
            ),
        ]
        ...
        result = _merge_intervals(intervals)
        assert len(result) == 3
        # Should have: on (0-20), off (20-40), on (40-80)

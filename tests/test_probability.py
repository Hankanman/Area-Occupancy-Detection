...
def test_sensor_type_weighting():
    ...
    result = calc.calculate_weighted_probability(
        [motion_prob, env_prob], [motion_weight, env_weight]
    )
...
def test_probability_stability():
    ...
    varied_probs = [
        [p + np.random.uniform(-0.01, 0.01) for p in probs] for _ in range(10)
    ]
...
def test_confidence_thresholds():
    ...
    high_conf_result = calc.calculate_weighted_probability(
        high_conf_probs, high_conf_weights
    )
    ...
    low_conf_result = calc.calculate_weighted_probability(
        low_conf_probs, low_conf_weights
    )
...
def test_probability_bounds():
    ...
    result = calc.calculate_weighted_probability([0.7, 0.6, 0.8, 0.5], extreme_weights)

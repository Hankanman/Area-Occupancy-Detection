...
async def test_coordinator_setup(hass: HomeAssistant, mock_fully_setup_entry):
    ...
    coordinator = hass.data["room_occupancy"][mock_fully_setup_entry.entry_id]["coordinator"]
...
async def test_coordinator_sensor_probability(hass: HomeAssistant):
    ...
    prob = coordinator._get_sensor_probability("binary_sensor.test_motion", motion_state)
...
async def test_coordinator_update_data(hass: HomeAssistant):
    ...
    with patch.object(coordinator, "_calculate_occupancy", side_effect=Exception("Test error")):

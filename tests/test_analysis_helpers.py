...
class TestCalculateTimeSlot:
    ...
    def test_slot_0_midnight(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot at midnight."""
        midnight = freeze_time.replace(hour=0, minute=0, second=0, microsecond=0)
        result = calculate_time_slot(midnight, slot_minutes=60)
        assert result == 0

    def test_various_hours_of_day(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot at various hours of day."""
        base = freeze_time.replace(minute=0, second=0, microsecond=0)

        assert calculate_time_slot(base.replace(hour=0), slot_minutes=60) == 0
        assert calculate_time_slot(base.replace(hour=1), slot_minutes=60) == 1
        assert calculate_time_slot(base.replace(hour=12), slot_minutes=60) == 12
        assert calculate_time_slot(base.replace(hour=23), slot_minutes=60) == 23

    def test_slot_boundaries(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot at slot boundaries."""
        base = freeze_time.replace(hour=10, second=0, microsecond=0)

        # At hour start (minute 0) - slot 10
        assert calculate_time_slot(base.replace(minute=0), slot_minutes=60) == 10
        # At minute 59 - still slot 10
        assert calculate_time_slot(base.replace(minute=59), slot_minutes=60) == 10
        # At next hour start - slot 11
        assert (
            calculate_time_slot(base.replace(hour=11, minute=0), slot_minutes=60) == 11
        )

    def test_different_slot_minutes_15(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with 15-minute slots."""
        base = freeze_time.replace(hour=10, second=0, microsecond=0)

        assert (
            calculate_time_slot(base.replace(minute=0), slot_minutes=15) == 40
        )  # 10 * 60 / 15 = 40
        assert calculate_time_slot(base.replace(minute=14), slot_minutes=15) == 40
        assert calculate_time_slot(base.replace(minute=15), slot_minutes=15) == 41
        assert calculate_time_slot(base.replace(minute=29), slot_minutes=15) == 41

    def test_different_slot_minutes_30(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with 30-minute slots."""
        base = freeze_time.replace(hour=10, second=0, microsecond=0)

        assert (
            calculate_time_slot(base.replace(minute=0), slot_minutes=30) == 20
        )  # 10 * 60 / 30 = 20
        assert calculate_time_slot(base.replace(minute=29), slot_minutes=30) == 20
        assert calculate_time_slot(base.replace(minute=30), slot_minutes=30) == 21

    def test_different_slot_minutes_120(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with 120-minute (2-hour) slots."""
        base = freeze_time.replace(hour=10, minute=0, second=0, microsecond=0)

        assert calculate_time_slot(base, slot_minutes=120) == 5  # 10 * 60 / 120 = 5
        # Test 119 minutes into the slot (10:00 + 119 min = 11:59)
        time_119_min = base + timedelta(minutes=119)
        assert calculate_time_slot(time_119_min, slot_minutes=120) == 5
        assert (
            calculate_time_slot(base.replace(hour=12, minute=0), slot_minutes=120) == 6
        )

    def test_edge_cases_end_of_day(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot at end of day."""
        end_of_day = freeze_time.replace(
            hour=23, minute=59, second=59, microsecond=999999
        )
        result = calculate_time_slot(end_of_day, slot_minutes=60)
        assert result == 23

    def test_slot_with_minutes_in_middle(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with minutes in the middle of an hour."""
        base = freeze_time.replace(hour=14, minute=30, second=0, microsecond=0)
        result = calculate_time_slot(base, slot_minutes=60)
        # 14:30 should still be in slot 14 with 60-minute slots
        assert result == 14


class TestAggregateIntervalsBySlot:
    """Test aggregate_intervals_by_slot function."""

    def test_empty_intervals(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with empty intervals."""
        result = aggregate_intervals_by_slot([], slot_minutes=60)
        assert result == []

    def test_single_interval_within_one_slot(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with single interval within one slot."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [(now, now + timedelta(minutes=30))]
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        assert len(result) == 1
        day, slot, seconds = result[0]
        assert isinstance(day, int)
        assert isinstance(slot, int)
        assert seconds == 1800.0  # 30 minutes in seconds

    def test_interval_spanning_multiple_slots(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with interval spanning multiple slots."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [(now, now + timedelta(hours=2, minutes=30))]
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should span 3 slots: 0-60min, 60-120min, 120-150min
        assert len(result) == 3
        total_seconds = sum(seconds for _, _, seconds in result)
        assert total_seconds == 9000.0  # 2.5 hours = 9000 seconds

    def test_interval_spanning_multiple_days(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with interval spanning multiple days."""
        now = freeze_time.replace(hour=23, minute=0, second=0, microsecond=0)
        intervals = [(now, now + timedelta(hours=2))]  # 23:00 to 01:00 next day
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should have entries for both days
        assert len(result) >= 2
        # Find slots for both days
        days = {day for day, _, _ in result}
        assert len(days) >= 1  # At least one day

    def test_multiple_intervals_same_slot_aggregation(
        self, freeze_time: datetime
    ) -> None:
        """Test aggregate_intervals_by_slot aggregates multiple intervals in same slot."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [
            (now, now + timedelta(minutes=20)),
            (now + timedelta(minutes=30), now + timedelta(minutes=50)),
        ]
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should aggregate both intervals into same slot
        assert len(result) == 1
        _, _, seconds = result[0]
        assert seconds == 2400.0  # 20 + 20 minutes = 2400 seconds

    def test_different_weekdays(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with intervals on different weekdays."""
        # Get Monday and Tuesday
        base = freeze_time.replace(hour=10, minute=0, second=0, microsecond=0)
        days_ahead_to_monday = (7 - base.weekday()) % 7
        if days_ahead_to_monday == 7:
            days_ahead_to_monday = 0
        monday = base + timedelta(days=days_ahead_to_monday)
        tuesday = monday + timedelta(days=1)

        intervals = [
            (monday, monday + timedelta(hours=1)),
            (tuesday, tuesday + timedelta(hours=1)),
        ]
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should have entries for both days
        assert len(result) == 2
        days = {day for day, _, _ in result}
        assert 0 in days or 1 in days  # Monday (0) or Tuesday (1)

    def test_slot_boundary_crossing(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with interval crossing slot boundary."""
        now = freeze_time.replace(minute=58, second=0, microsecond=0)
        intervals = [(now, now + timedelta(minutes=5))]  # 58min to 63min
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should span 2 slots: minutes 58-60 (slot N) and 60-63 (slot N+1)
        assert len(result) == 2
        total_seconds = sum(seconds for _, _, seconds in result)
        assert total_seconds == 300.0  # 5 minutes = 300 seconds
...
class TestCalculateTimeSlotEdgeCases:
    """Test additional edge cases for calculate_time_slot."""

    def test_midnight_boundary(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot at midnight (00:00)."""
        midnight = freeze_time.replace(hour=0, minute=0, second=0, microsecond=0)
        result = calculate_time_slot(midnight, slot_minutes=60)
        assert result == 0

    def test_just_before_midnight(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot just before midnight."""
        before_midnight = freeze_time.replace(
            hour=23, minute=59, second=59, microsecond=999999
        )
        result = calculate_time_slot(before_midnight, slot_minutes=60)
        assert result == 23

    def test_small_slot_minutes(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with very small slot_minutes."""
        now = freeze_time.replace(hour=10, minute=30, second=0, microsecond=0)
        # 1 minute slots
        result = calculate_time_slot(now, slot_minutes=1)
        assert result == 630  # 10*60 + 30 = 630

    def test_large_slot_minutes(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with large slot_minutes."""
        now = freeze_time.replace(hour=12, minute=0, second=0, microsecond=0)
        # 240 minute (4 hour) slots
        result = calculate_time_slot(now, slot_minutes=240)
        assert result == 3  # 12*60 / 240 = 3

    def test_slot_minutes_equaling_day(self, freeze_time: datetime) -> None:
        """Test calculate_time_slot with slot_minutes equal to full day."""
        now = freeze_time
        result = calculate_time_slot(now, slot_minutes=1440)  # 24*60 = 1440
        assert result == 0  # Everything in first (and only) slot


class TestAggregateIntervalsBySlotEdgeCases:
    """Test additional edge cases for aggregate_intervals_by_slot."""

    def test_zero_duration_interval(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with zero duration interval."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [(now, now)]  # Zero duration
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Zero duration should result in no aggregation
        assert result == []

    def test_interval_at_week_boundary(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with interval spanning week boundary."""
        # Get Sunday 23:00
        base = freeze_time.replace(hour=23, minute=0, second=0, microsecond=0)
        # Move to Sunday
        days_to_sunday = (6 - base.weekday()) % 7
        if days_to_sunday == 7:
            days_to_sunday = 0
        sunday = base + timedelta(days=days_to_sunday)

        intervals = [
            (sunday, sunday + timedelta(hours=2))
        ]  # Sunday 23:00 to Monday 01:00
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should have entries for both Sunday and Monday
        days = {day for day, _, _ in result}
        assert 6 in days or 0 in days  # Sunday (6) or Monday (0)

    def test_interval_spanning_multiple_days_within_week(
        self, freeze_time: datetime
    ) -> None:
        """Test aggregate_intervals_by_slot with interval spanning multiple days."""
        # Get Monday 23:00
        ...
        intervals = [
            (monday, monday + timedelta(hours=50))
        ]  # Monday 23:00 to Wednesday 01:00
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should span Monday, Tuesday, and Wednesday
        days = {day for day, _, _ in result}
        assert len(days) >= 2

    def test_very_small_interval(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with very small interval (microseconds)."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [(now, now + timedelta(microseconds=1000))]
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should still be aggregated
        assert isinstance(result, list)
        if result:
            assert sum(seconds for _, _, seconds in result) > 0

    def test_multiple_intervals_different_weeks(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with intervals from different weeks."""
        # Get Monday of current week
        base = freeze_time.replace(hour=10, minute=0, second=0, microsecond=0)
        days_ahead_to_monday = (7 - base.weekday()) % 7
        if days_ahead_to_monday == 7:
            days_ahead_to_monday = 0
        monday_week1 = base + timedelta(days=days_ahead_to_monday)
        monday_week2 = monday_week1 + timedelta(days=7)

        intervals = [
            (monday_week1, monday_week1 + timedelta(hours=1)),
            (monday_week2, monday_week2 + timedelta(hours=1)),
        ]
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        # Should aggregate both into same slot (same day of week, same slot)
        assert len(result) >= 1

    def test_interval_at_slot_boundary(self, freeze_time: datetime) -> None:
        """Test aggregate_intervals_by_slot with interval exactly at slot boundary."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [(now, now + timedelta(minutes=60))]  # Exactly one slot
        result = aggregate_intervals_by_slot(intervals, slot_minutes=60)
        assert len(result) == 1
        _, _slot, seconds = result[0]
        assert seconds == 3600.0  # Exactly 1 hour = 3600 seconds

    def test_interval_spanning_multiple_slots_same_hour(
        self, freeze_time: datetime
    ) -> None:
        """Test aggregate_intervals_by_slot with interval spanning multiple small slots."""
        now = freeze_time.replace(minute=0, second=0, microsecond=0)
        intervals = [(now, now + timedelta(minutes=30))]  # 30 minutes
        result = aggregate_intervals_by_slot(intervals, slot_minutes=15)  # 15 min slots
        # Should span 2 slots (0-15, 15-30)
        assert len(result) == 2
        total_seconds = sum(seconds for _, _, seconds in result)
        assert total_seconds == 1800.0  # 30 minutes = 1800 seconds

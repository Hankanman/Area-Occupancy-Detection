...
from custom_components.room_occupancy.const import (
    DOMAIN,
    CONF_DECAY_TYPE,  # Add this import
)
...
async def test_setup_entry_fails_not_ready(hass: HomeAssistant, mock_config_entry):
    ...
    entry = MockConfigEntry(
        domain=DOMAIN, data=mock_config_entry, entry_id="test_entry_id"
    )
...
async def test_setup_entry_migration(hass: HomeAssistant, mock_config_entry):
    ...
    entry = MockConfigEntry(
        domain=DOMAIN, data=old_config, entry_id="test_entry_id", version=1
    )
    entry.add_to_hass(hass)

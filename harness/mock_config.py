...
def build(
    ...
    Args:
        ...
        port: Port the instance listens on. Not written here -- since 2026.9
            an ``http:`` block is migrated into the HTTP config store as an
            unconfirmed trial that reverts after five minutes, so
            ``storage.write_http_config`` seeds it as the confirmed config
            instead. Kept as a parameter because the rendered header names
            the port, and to keep the two in one place.
...
def render(profile: Profile, *, time_zone: str, frontend: bool, port: int) -> str:
    ...
    return (
        ...
        f"# Listens on port {port}, pinned in .storage/http.\n"
        "#\n"

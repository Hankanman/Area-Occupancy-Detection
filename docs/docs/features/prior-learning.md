...
Initially, the integration uses default prior values. However, the **Prior Learning** process aims to calculate a more accurate prior based on the historical behavior of your **motion sensors**, reflecting how often that area is typically occupied.
...
3.  **`P(Sensor Occupied)` (Prior):** What is the baseline probability that this sensor is active, based on its history?

## Motion Sensors as Ground Truth

This learning process uses **motion sensors** as the "ground truth" indicator of when the area was actually occupied during the historical analysis period. All configured motion sensors are used together to determine occupied intervals.
...
2.  **Data Retrieval:** For the specified period, it fetches the state history for all **motion sensors** and the specific **sensor being analyzed**.
...
4.  **Correlation Calculation:** The system compares the time intervals of the sensor being analyzed with the intervals when **motion sensors** indicated the area was `on` (considered occupied) and `off` (considered not occupied):
    - **Calculate `P(Active | Occupied)`:** It measures the total duration the sensor was _active_ during the times motion sensors indicated the area was _occupied_. This duration is divided by the total time the area was considered _occupied_.
    - **Calculate `P(Active | Not Occupied)`:** It measures the total duration the sensor was _active_ during the times motion sensors indicated the area was _not occupied_. This duration is divided by the total time the area was considered _not occupied_.
    - **Calculate `P(Occupied)` (Prior):** The prior is calculated based on the total time motion sensors indicated the area was occupied, divided by the total analysis period duration. For other sensors, their individual priors are also calculated based on their own total active time.
